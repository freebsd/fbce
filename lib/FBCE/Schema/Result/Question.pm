use utf8;
package FBCE::Schema::Result::Question;
...
# DO NOT MODIFY THIS OR ANYTHING ABOVE! md5sum:mWPs3n5YMUMwacujubemDw

=head2 votes

Return votes cast for this question.  In list context, returns a list
of votes.  In scalar context, returns a resultset.
...
Return votes cast for this question as a resultset.
...
sub commit_answer {
    ...
    $voter->poll_votes->search({ question => $self->id })->delete();
    foreach my $oid (@answer) {
	$voter->poll_votes->create({ question => $self->id, option => $oid });
    }
}
