...
FBCE::Controller::Poll - Catalyst Controller
...
sub poll :Chained('/') :Path :CaptureArgs(Int) {
    my ($self, $c, $pid) = @_;

    $self->require_user($c);
    my $poll = $c->model('FBCE::Poll')->find($pid);
    $c->detach('/default')
	...
	foreach my $question ($poll->questions) {
	    my $votes = $c->user->poll_votes->search({ question => $question->id });
...
sub review :Chained('poll') :Path :Args(0) {
    ...
    my $options = $c->model('FBCE::Option');
